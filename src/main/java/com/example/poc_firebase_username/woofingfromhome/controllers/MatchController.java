...
public class MatchController {
    ...
    public ResponseEntity<List<Match>> getAllMatches(@RequestParam(required = false, name = "firebaseId") String firebaseId) {
        if (firebaseId != null) {
            return new ResponseEntity<>(matchRepository.findByCustomer1_FirebaseIdAndScoreGreaterThanOrCustomer2_FirebaseIdOrderByDistance(firebaseId, 0, firebaseId), HttpStatus.OK);
        }
